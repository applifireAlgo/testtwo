...
@SourceCodeAuthorClass(createdBy = "anagha", updatedBy = "", versionNumber = "1", comments = "Service for State Master table Entity", complexity = Complexity.LOW)
...
public class StateServiceImpl extends StateService {
    ...
    public HttpEntity<ResponseBean> findAll() throws SpartanPersistenceException, Exception {
        ...
        java.util.List<test.app.shared.location.State> lststate = staterepo.findAll();
    ...
    public HttpEntity<ResponseBean> findByCountryId(@RequestBody FindByBean findByBean) throws SpartanPersistenceException, Exception {
        ...
        org.springframework.http.HttpStatus httpStatus = org.springframework.http.HttpStatus.OK;
        try {
            List<test.app.shared.location.State> lststate = staterepo.findByCountryId((java.lang.String) findByBean.getFindKey());
            responseBean.add("success", true);
            responseBean.add("message", "Successfully retrived ");
            responseBean.add("data", lststate);
        } catch (org.springframework.transaction.TransactionException e) {
    ...
    public HttpEntity<ResponseBean> findById(@RequestBody FindByBean findByBean) throws SpartanPersistenceException, Exception {
        ...
        org.springframework.http.HttpStatus httpStatus = org.springframework.http.HttpStatus.OK;
        try {
            test.app.shared.location.State lststate = staterepo.findById((java.lang.String) findByBean.getFindKey());
            responseBean.add("success", true);
            responseBean.add("message", "Successfully retrived ");
            responseBean.add("data", lststate);
        } catch (org.springframework.transaction.TransactionException e) {
