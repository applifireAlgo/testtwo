...
@SourceCodeAuthorClass(createdBy = "anagha", updatedBy = "", versionNumber = "1", comments = "Repository for Roles Transaction table", complexity = Complexity.MEDIUM)
public class RolesRepositoryImpl extends SearchInterfaceImpl implements RolesRepository<Roles> {
    ...
    public List<Roles> findAll() throws SpartanPersistenceException {
        try {
            ...
            java.util.List<test.app.shared.authorization.Roles> query = emanager.createQuery("select u from Roles u where u.systemInfo.activeStatus=1").getResultList();
            return query;
    ...
    public List<Roles> save(List<Roles> entity) throws SpartanPersistenceException {
        try {
            ...
            for (int i = 0; i < entity.size(); i++) {
                test.app.shared.authorization.Roles obj = entity.get(i);
    ...
    public void delete(String id) throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            test.app.shared.authorization.Roles s = emanager.find(test.app.shared.authorization.Roles.class, id);
            emanager.remove(s);
        } catch (javax.persistence.PersistenceException e) {
    ...
    public void deleteRoleMenuBridge(List<RoleMenuBridge> rolemenubridge) throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            for (test.app.shared.authorization.RoleMenuBridge _rolemenubridge : rolemenubridge) {
                test.app.shared.authorization.RoleMenuBridge s = emanager.find(test.app.shared.authorization.RoleMenuBridge.class, _rolemenubridge.getRoleMenuMapId());
                emanager.remove(s);
            }
        } catch (javax.persistence.PersistenceException e) {
            throw new SpartanPersistenceException("Error in deleting entity", e);
    ...
    public Roles findById(String roleId) throws Exception, SpartanPersistenceException {
        try {
            ...
            test.app.shared.authorization.Roles listOfRoles = (test.app.shared.authorization.Roles) query.getSingleResult();
            return listOfRoles;
        } catch (javax.persistence.PersistenceException e) {
