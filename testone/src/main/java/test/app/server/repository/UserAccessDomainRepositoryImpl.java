...
@SourceCodeAuthorClass(createdBy = "anagha", updatedBy = "", versionNumber = "1", comments = "Repository for UserAccessDomain Master table Entity", complexity = Complexity.LOW)
public class UserAccessDomainRepositoryImpl extends SearchInterfaceImpl implements UserAccessDomainRepository<UserAccessDomain> {
    ...
    public List<UserAccessDomain> findAll() throws SpartanPersistenceException {
        try {
            ...
            java.util.List<test.app.shared.authentication.UserAccessDomain> query = emanager.createQuery("select u from UserAccessDomain u where u.systemInfo.activeStatus=1").getResultList();
            return query;
    ...
    public List<UserAccessDomain> save(List<UserAccessDomain> entity) throws SpartanPersistenceException {
        try {
            ...
            for (int i = 0; i < entity.size(); i++) {
                test.app.shared.authentication.UserAccessDomain obj = entity.get(i);
    ...
    public void delete(String id) throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            test.app.shared.authentication.UserAccessDomain s = emanager.find(test.app.shared.authentication.UserAccessDomain.class, id);
            emanager.remove(s);
        } catch (javax.persistence.PersistenceException e) {
    ...
    public UserAccessDomain findById(String userAccessDomainId) throws Exception, SpartanPersistenceException {
        try {
            ...
            test.app.shared.authentication.UserAccessDomain listOfUserAccessDomain = (test.app.shared.authentication.UserAccessDomain) query.getSingleResult();
