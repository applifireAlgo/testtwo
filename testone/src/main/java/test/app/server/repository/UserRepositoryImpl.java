...
import test.app.shared.authentication.PassRecovery;
...
public class UserRepositoryImpl extends SearchInterfaceImpl implements UserRepository<User> {
    ...
    public List<User> findAll() throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            java.util.List<test.app.shared.authentication.User> query = emanager.createQuery("select u from User u where u.systemInfo.activeStatus=1").getResultList();
            return query;
        } catch (javax.persistence.PersistenceException e) {
    ...
    public List<User> save(List<User> entity) throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            for (int i = 0; i < entity.size(); i++) {
                test.app.shared.authentication.User obj = entity.get(i);
    ...
    public void delete(String id) throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            test.app.shared.authentication.User s = emanager.find(test.app.shared.authentication.User.class, id);
            emanager.remove(s);
        } catch (javax.persistence.PersistenceException e) {
    ...
    public void deletePassRecovery(List<PassRecovery> passrecovery) throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            for (test.app.shared.authentication.PassRecovery _passrecovery : passrecovery) {
                test.app.shared.authentication.PassRecovery s = emanager.find(test.app.shared.authentication.PassRecovery.class, _passrecovery.getPassRecoveryId());
                emanager.remove(s);
            }
        } catch (javax.persistence.PersistenceException e) {
            throw new SpartanPersistenceException("Error in deleting entity", e);
    ...
    public List<User> findByUserAccessLevelId(String userAccessLevelId) throws Exception, SpartanPersistenceException {
        try {
            ...
            java.util.List<test.app.shared.authentication.User> listOfUser = query.getResultList();
            return listOfUser;
        } catch (javax.persistence.PersistenceException e) {
            throw new com.athena.framework.server.exception.repository.SpartanPersistenceException("Error in executing query", e);
        }
    ...
    public User findById(String userId) throws Exception, SpartanPersistenceException {
        try {
            ...
            javax.persistence.Query query = emanager.createNamedQuery("User.findById");
            query.setParameter("userId", userId);
            test.app.shared.authentication.User listOfUser = (test.app.shared.authentication.User) query.getSingleResult();
            return listOfUser;
        } catch (javax.persistence.PersistenceException e) {
            throw new com.athena.framework.server.exception.repository.SpartanPersistenceException("Error in executing query", e);
        }
